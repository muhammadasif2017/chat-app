...
|---|---|---|---|
| Access token | `JWT_SECRET` | 15 min | `HttpOnly; SameSite=Lax; Secure` cookie |
| Refresh token | `JWT_REFRESH_SECRET` | 7 days | `HttpOnly; SameSite=Lax; Secure` cookie (path `/auth/refresh`) + bcrypt hash in DB |
...
- **`RefreshToken` table grows** — tokens are deleted on use and on logout, but
  expired-but-unused tokens accumulate. `CleanupService` runs hourly via
  `@nestjs/schedule` and prunes rows where `expiresAt < now`. An index on
  `expiresAt` keeps this query fast as the table grows.
